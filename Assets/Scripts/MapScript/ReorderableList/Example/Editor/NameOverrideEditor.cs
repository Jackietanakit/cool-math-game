...
namespace Map
{
    ...
    public class NameOverrideEditor : Editor
    {
        ...
        private void UpdateNestedElementNames(
            SerializedProperty array,
            SerializedProperty nameOverride
        )
        {
            for (int i = 0; i < array.arraySize; i++)
            {
                UpdateElementNames(
                    array.GetArrayElementAtIndex(i).FindPropertyRelative("nested"),
                    nameOverride
                );
            }
        }
        ...
        {
            ReorderableList list = ReorderableDrawer.GetList(
                listProperty,
                ReorderableDrawer.ARRAY_PROPERTY_NAME
            );
